...
3. Статическую роль, которую бренд задаёт сам (кроме `font-sans`), перечислите в `_ownStatics` файла бренда: иначе `npm run tokens:import` вернёт ей значение `maps`.
4. `npm run tokens:build` перегенерирует CSS и этот файл; `npm test` не пропустит пропущенную роль или `var()` в значении.
