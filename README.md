...
Набор иконок макета: 104 заливных глифа в поле 24×24, три раздела — Interface, Map UI,
Weather.
...
Поля: `TextInput`, `PasswordInput`, `SearchInput` (обводка, иконка и ввод; отправку вешает
потребитель — `onKeyDown` у поля или `<form>` вокруг него), `PhoneInput`, `CodeInput`,
`SelectField`, `DayPicker`.
...
У каждого компонента две истории: **All Variants** — все состояния макета статикой, и
**Playground** — один экземпляр с контролами. Переключатели бренда × темы в тулбаре действуют на
любую историю. По умолчанию канва — фон страницы Figma (`#a7a7a7` + белый 25 %), чтобы сравнивать
компонент с макетом один в один; переключатель Canvas возвращает фон бренда:
