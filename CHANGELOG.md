...
- `MenuTabs` — новый компонент по `Menu Toggle Item` (`251:758`): заголовок 12/16 весом 500
  ...
  «Menu Toggle Item», но переключателя в нём нет — это те же вкладки с подписью.
...
- `NavBar` работает и без внешнего состояния: с `defaultActiveId` панель сама отмечает нажатую
  вкладку и всё так же зовёт `onSelect`. С `activeId` поведение прежнее — вкладку выбирает
  приложение.
- **Ломающее.** `MapCompass`: буква N вращается вместе со шкалой — прежде она лежала отдельным
  неподвижным слоем и оставалась стоять, пока шкала крутилась. Тень осталась только на шкале,
  как в макете `109:612`.
- `MapCompass` работает и без внешнего состояния: с `defaultHeading` компас сам возвращается на
  север по нажатию. С `heading` поведение прежнее — поворот решает приложение. Проп `aligned`
  стал необязательным: без него состояние выводится из азимута.
