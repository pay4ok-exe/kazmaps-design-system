...
- `PhoneInput` по макету `148:721`: селектор показывает флаг и шеврон, код страны набран в поле перед
  номером и назван в подписи кнопки региона, маска группами через пробел. Принимает `ref` на поле и `aria-describedby`, `aria-label`, `aria-labelledby`: текст ошибки рядом с полем связывается через `aria-describedby`. Флаги регионов — эмодзи для
